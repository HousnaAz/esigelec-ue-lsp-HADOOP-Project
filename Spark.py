from pyspark.sql.functions import monotonically_increasing_id, col, lag, trim, substring, when, udf,sqrt, pow, round,sum
...
df = df.withColumn("goal",when((substring(trim(df.prev_state),1,10).contains('n')==False) & (substring(trim(df.state),1,10).contains('n')) & ( df.prev_y>150) & (df.prev_y<300)& ((df.prev_x < 100) | (df.prev_x >500)) , 1).otherwise(0))
